...
import { calculateRiskScore } from '@engine/rules/risk'
import { generateImpactReports } from '@engine/reporters/impact'
import type { DiffResult, OpenAPIContract } from '@engine/models/types'
import type { RiskScore } from '@engine/rules/risk'
import type { ImpactReport } from '@engine/reporters/impact'
...
export function generateHTMLReport(diffResult: RunDiffResult): string {
  ...
  function impactBlock(r: ImpactReport): string {
    return `<div style="padding:16px 20px;border-bottom:1px solid #27272a">
      ...
      <ul style="padding-left:16px;color:#a1a1aa;font-size:13px">${r.impacts.map(i => `<li style="margin-bottom:4px">${i}</li>`).join('')}</ul>
    </div>`
  }
...
<div class="section" style="margin-bottom:24px">
  ...
  ${riskScore.breakdown.length > 0 ? `<div class="bdwn">${riskScore.breakdown.map(b => {
...
<footer><p>Generated by API Contract Diff Engine v1.0.0</p></footer>
</div></body></html>`
}
