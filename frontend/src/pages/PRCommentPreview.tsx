...
import { MessageSquare, Eye, Code2, Copy, CheckCheck, AlertCircle } from 'lucide-react'
        ...
        <div className="lg:col-span-3">
          {mode === 'off' ? (
            <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-10 text-center space-y-3">
              ...
              <p className="text-xs text-zinc-300 dark:text-zinc-600">No comment will be posted to the PR. Switch to summary or full to see a preview.</p>
            </div>
          ) : !result ? (
            <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-10 text-center">
              <p className="text-sm text-zinc-400 dark:text-zinc-500">Run an analysis first</p>
            </div>
          ) : view === 'raw' ? (
  ...
  return (
    <div className="text-sm text-zinc-800 dark:text-zinc-200 space-y-1.5 font-sans leading-relaxed">
      {lines.map((line, i) => {
        ...
        if (line.startsWith('| ')) return <div key={i} className="font-mono text-[11px] text-zinc-600 dark:text-zinc-400">{line}</div>
        if (line.startsWith('- ') || line.startsWith('* ')) return <div key={i} className="flex items-start gap-1.5 text-xs"><span className="text-zinc-400 mt-1">•</span><span>{line.slice(2)}</span></div>
...
function NoReport() {
  return (
    <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg px-5 py-4 text-sm text-amber-700 dark:text-amber-300 flex items-center gap-2">
      ...
      No report loaded. Run an analysis in Contract Playground to generate a real PR comment.
    </div>
  )
}
